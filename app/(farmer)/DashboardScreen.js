...
export default function DashboardScreen({ navigation }) {
  ...
  const getWeatherLabel = (code) => {
    if (code === 0) return t("dashboard.clearSky");
    if (code <= 2) return t("dashboard.partlyCloudy");
    if (code <= 3) return t("dashboard.overcast");
    if (code <= 49) return t("dashboard.foggy");
    if (code <= 59) return t("dashboard.drizzle");
    if (code <= 69) return t("dashboard.rainy");
    if (code <= 79) return t("dashboard.snow");
    if (code <= 84) return t("dashboard.rainShowers");
    if (code <= 99) return t("dashboard.thunderstorm");
    return "";
  };
